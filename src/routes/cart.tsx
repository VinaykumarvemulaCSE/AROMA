...
export const Route = createFileRoute("/cart")({
  head: () => ({ meta: [{ title: "Cart — Aroma Cafe" }, { name: "description", content: "Review your order before checkout." }] }),
  component: CartPage,
});
      ...
      <section className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 py-10 pb-32 lg:pb-10">
        ...
        {lines.length === 0 ? (
          <div className="mt-14 text-center bg-card border border-border rounded-2xl py-16 px-6">
            ...
            <Link to="/menu"><Button className="mt-5">Browse menu</Button></Link>
          </div>
        ) : (
          <div className="mt-8 grid lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-3">
              {lines.map((l) => (
                <div key={l.id} className="bg-card border border-border rounded-2xl p-3">
                  ...
                  <div className="flex items-start gap-3">
                    <img src={l.image} alt={l.name} className="size-16 sm:size-20 rounded-xl object-cover shrink-0" />
                    <div className="flex-1 min-w-0">
                      <p className="font-semibold truncate">{l.name}</p>
                      <p className="text-sm text-muted-foreground">{inr(l.price)} each</p>
                    </div>
                    <button onClick={() => remove(l.id)} className="text-muted-foreground hover:text-destructive p-2 shrink-0" aria-label="Remove"><Trash2 className="size-4" /></button>
                  </div>
                  {/* Bottom row: qty + line total */}
                  <div className="mt-3 flex items-center justify-between gap-3">
                    <div className="flex items-center border border-border rounded-full">
                      <button onClick={() => setQty(l.id, l.qty - 1)} className="size-9 grid place-items-center" aria-label="Decrease">−</button>
                      <span className="w-8 text-center text-sm font-semibold">{l.qty}</span>
                      <button onClick={() => setQty(l.id, l.qty + 1)} className="size-9 grid place-items-center" aria-label="Increase">+</button>
                    </div>
              ...
              <div className="flex flex-col sm:flex-row gap-2 sm:justify-between pt-2">
                <Link to="/menu"><Button variant="outline" className="w-full sm:w-auto">Continue shopping</Button></Link>
                <Button variant="ghost" onClick={clear} className="text-destructive w-full sm:w-auto">Clear cart</Button>
              </div>
            </div>
            ...
            <aside className="bg-card border border-border rounded-2xl p-6 h-fit lg:sticky lg:top-24">
              ...
              <dl className="mt-4 space-y-2 text-sm">
                ...
                {subtotal < 499 && <p className="text-xs text-muted-foreground">Add {inr(499 - subtotal)} more for free delivery.</p>}
                <div className="border-t border-border pt-3 mt-3 flex justify-between font-display font-bold text-lg"><span>Total</span><span>{inr(total)}</span></div>
              </dl>
              <Link to="/checkout"><Button size="lg" className="w-full mt-5">Proceed to checkout</Button></Link>
              <p className="text-xs text-muted-foreground mt-3 text-center">Pay via WhatsApp after order confirmation</p>
            </aside>
          </div>
        )}
      </section>
      ...
      {lines.length > 0 && (
        <div className="lg:hidden fixed bottom-16 inset-x-0 z-30 px-4 pb-3">
          <div className="mx-auto max-w-md bg-card border border-border rounded-2xl shadow-lg p-3 flex items-center justify-between gap-3">
            ...
            <Link to="/checkout" className="flex-1"><Button className="w-full">Checkout</Button></Link>
          </div>
        </div>
      )}
    </SiteLayout>
