  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">Approve or reject customer reviews before they appear on the site.</p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <Input placeholder="Search name, title, content..." value={q} onChange={(e) => setQ(e.target.value)} className="pl-9" />
        </div>
            ...
            <article key={r.id} className="bg-card border border-border rounded-2xl p-4 sm:p-5">
              <div className="flex items-start justify-between gap-3 flex-wrap">
                <div className="flex items-center gap-3 min-w-0">
                  <div className="size-10 grid place-items-center rounded-full bg-secondary font-display font-semibold shrink-0">{r.name[0]}</div>
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {r.status !== "approved" && (
                  <Button size="sm" onClick={() => handleSetStatus(r.id, "approved", "Review approved")}>
                    <Check className="size-4" /> Approve
                  </Button>
                )}
                {r.status !== "rejected" && (
                  <Button size="sm" variant="outline" onClick={() => handleSetStatus(r.id, "rejected", "Review rejected")}>
                    <X className="size-4" /> Reject
                  </Button>
                )}
                {r.status !== "pending" && (
                  <Button size="sm" variant="ghost" onClick={() => handleSetStatus(r.id, "pending", "Moved back to pending")}>
                    Move to pending
                  </Button>
                )}
