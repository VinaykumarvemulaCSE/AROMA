// Server-only email helpers (Nodemailer). Never import from client code.
// Reads credentials from process.env (SMTP_HOST, SMTP_PORT, SMTP_USER, SMTP_PASS, SMTP_FROM).

import { getAppUrl, isProductionRuntime } from "./config.server";
...
/** Throw when an internal email helper reported failure. */
export function assertEmailSent(result: EmailSendResult, label = "email") {
  if (!result.success) {
...
async function getTransporter() {
  ...
  if (!host || !user || !pass) {
    if (isProductionRuntime()) {
      throw new Error(
        "SMTP is not configured. Set SMTP_HOST, SMTP_USER, and SMTP_PASS in production.",
      );
    }
    // Local/dev only — never pretend success in production
    console.warn(
...
// 8. Contact Form Email (called from createServerFn in contact-email.ts)
export async function sendContactEmailInternal(data: {
  name: string;
  email: string;
  message: string;
}) {
