...
function TrackPageContent() {
  ...
  const waSearch = searchParams.get("wa") === "1" ? 1 : undefined;
  ...
  const fetchGuestOrder = useCallback(
    async (phone: string) => {
      setTrackError(null);
      try {
        const res = await getOrderForTracking({ orderId, phone });
        if (!res.success || !res.order) {
          setTrackError(res.error || "Could not load order.");
          setTrackedOrder((prev) => {
            if (!prev) setNeedsPhone(true);
            return prev;
          });
          return;
        }
