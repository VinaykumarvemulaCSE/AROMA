...
//#region node_modules/.nitro/vite/services/ssr/assets/email-CMJXOQcA.js
...
var inrFormat = (n) => `₹${n.toLocaleString("en-IN")}`;
async function sendOrderEmailInternal(data) {
	...
	let adminMailPromise = Promise.resolve(null);
	if (adminEmail) adminMailPromise = transporter.sendMail({
...
async function sendReservationEmailInternal(data) {
	const transporter = await getTransporter();
	const adminEmail = process.env.ADMIN_EMAIL || process.env.SMTP_FROM || "";
	const locationLink = data.location ? `<p style="margin: 4px 0;"><strong>Location:</strong> <a href="https://www.google.com/maps/dir/?api=1&destination=${data.location.latitude},${data.location.longitude}&destination_place_id=${data.location.placeId}">${data.location.address}</a></p>` : "";
	const htmlContent = `
    <div style="font-family: 'Inter', system-ui, -apple-system, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 16px; background-color: #ffffff; color: #1a202c;">
      <div style="text-align: center; padding-bottom: 20px; border-bottom: 2px solid #f7fafc;">
        <h1 style="font-size: 24px; font-weight: 800; color: #854d0e; margin: 0;">Aroma Cafe & Restaurant</h1>
  ...
  `;
	const fromAddress = process.env.SMTP_FROM || "no-reply@aromacafe.in";
	let userMailPromise = Promise.resolve(null);
	if (data.customerEmail) userMailPromise = transporter.sendMail({
		from: `"Aroma Cafe" <${fromAddress}>`,
		to: data.customerEmail,
...
async function sendOrderStatusEmailInternal(data) {
	const transporter = await getTransporter();
	const fromAddress = process.env.SMTP_FROM || "no-reply@aromacafe.in";
	...
	const itemsList = data.items.map((item) => `<li style="padding: 4px 0; border-bottom: 1px solid #f0f0f0;">
...
async function sendVerificationEmailInternal(data) {
	const transporter = await getTransporter();
	const fromAddress = process.env.SMTP_FROM || "no-reply@aromacafe.in";
	const htmlContent = `
    <div style="font-family: 'Inter', system-ui, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 16px; background: #fff; color: #1a202c;">
...
async function sendReservationStatusEmailInternal(data) {
	const transporter = await getTransporter();
	const fromAddress = process.env.SMTP_FROM || "no-reply@aromacafe.in";
	const statusMessages = {
		"Pending": {
    ...
    <div style="font-family: 'Inter', system-ui, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 16px; background: #fff; color: #1a202c;">
      ...
      <div style="background-color: #f7fafc; padding: 15px; border-radius: 8px; margin: 20px 0;">
        ...
        <p style="font-size: 14px; margin: 0; color: #2d3748;"><strong>Status:</strong> ${data.status}</p>
        <p style="font-size: 14px; margin: 6px 0 0 0; color: #2d3748;"><strong>Date:</strong> ${new Date(data.date).toLocaleDateString("en-IN", {
		weekday: "short",
		day: "numeric",
		month: "short"
	})}</p>
...
async function sendOrderCancellationEmailInternal(data) {
	const transporter = await getTransporter();
	const fromAddress = process.env.SMTP_FROM || "no-reply@aromacafe.in";
	const itemsList = data.items.map((item) => `<li style="padding: 4px 0; border-bottom: 1px solid #f0f0f0;">
      <span style="font-weight: 500;">${item.name}</span> × ${item.qty} - ${inrFormat(item.price * item.qty)}
...
async function sendReviewApprovalEmailInternal(data) {
	const transporter = await getTransporter();
	const fromAddress = process.env.SMTP_FROM || "no-reply@aromacafe.in";
	const htmlContent = `
    <div style="font-family: 'Inter', system-ui, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 16px; background: #fff; color: #1a202c;">
      ...
      <div style="background-color: #f7fafc; padding: 15px; border-radius: 8px; margin: 20px 0;">
        <div style="font-size: 24px; margin-bottom: 8px;">${"⭐".repeat(data.rating)}</div>
...
var sendContactEmail = createServerFn({ method: "POST" }).validator((data) => contactMessageSchema.parse(data)).handler(createSsrRpc("8ea8690aaab80875cf482a126a44cd9f13ec49afc9b16a0316a32eb799705071"));
//#endregion
export { sendReservationEmailInternal as a, sendVerificationEmailInternal as c, sendOrderStatusEmailInternal as i, sendOrderCancellationEmailInternal as n, sendReservationStatusEmailInternal as o, sendOrderEmailInternal as r, sendReviewApprovalEmailInternal as s, sendContactEmail as t };
